...
const Accounts = () => {
  ...
  const [loading, isLoading] = useState(true);
    ...
    const getAccounts = async () => {
      try {
        const res = await fetch(`/api/accounts?name=${encodeURIComponent(searchTerm)}`);
  ...
  const recordVoiceSample = async (userId) => {
    try {
      ...
      newRecorder.onstop = () => {
        ...
        localStorage.setItem(`voiceSample-${userId}`, url);
        ...
        <div className="flex justify-center items-center h-screen">
          <form
            onSubmit={async (e) => {
              ...
              setError('');
              try {
                const res = await fetch(`/api/address-book?name=${encodeURIComponent(searchTerm)}`);
                const data = await res.json();
                if (data && data.found) {
                  setSearchResult(data.accounts);
                  setAccountFound(true);
                } else {
                  setSearchResult(null);
                  setError('Did not find address book');
          ...
          >
            <div className="flex gap-2">
              <input
                ...
                placeholder="Search address book by name..."
